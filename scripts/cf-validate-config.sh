...
require_binding() {
  ...
  local description="$3"
  if ! grep -qP "^[[:space:]]*${section_pattern}[[:space:]]*$" "${CONFIG_PATH}"; then
    echo "❌ Missing ${description} section in ${CONFIG_PATH}" >&2
    exit 1
  fi
  if ! grep -qP "^[[:space:]]*${binding_pattern}[[:space:]]*$" "${CONFIG_PATH}"; then
...
if ! grep -qP 'NEXT_PUBLIC_SITE_URL[[:space:]]*=[[:space:]]*"https?://[^"]+"' "${CONFIG_PATH}"; then
  echo "❌ Missing NEXT_PUBLIC_SITE_URL in ${CONFIG_PATH}" >&2
  exit 1
fi
if grep -qP 'NEXT_PUBLIC_SITE_URL[[:space:]]*=[[:space:]]*"https?://(localhost|127\.0\.0\.1|0\.0\.0\.0|example\.com)(:[0-9]+)?/?"' "${CONFIG_PATH}"; then
